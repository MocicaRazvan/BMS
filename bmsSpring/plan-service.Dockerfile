...
# ---------------------- PLAN SERVICE  ----------------------
FROM maven-base AS plan-service
...
RUN mvn -f plan-service/pom.xml dependency:go-offline -B -q
COPY plan-service/src plan-service/src
RUN mvn -q -f plan-service/pom.xml clean package -DskipTests -Dmaven.compiler.showWarnings=true -Dmaven.compiler.source=22 -Dmaven.compiler.target=22 -Dmaven.compiler.compilerArgs=--enable-preview
...
COPY --from=plan-service \
     --chown=appuser:appgroup \
     /app/plan-service/target/plan-service-0.0.1-SNAPSHOT.jar \
     plan-service-0.0.1-SNAPSHOT.jar

USER appuser:appgroup


ENTRYPOINT ["java", "--enable-preview", "-Dreactor.schedulers.defaultBoundedElasticOnVirtualThreads=true","-jar", "plan-service-0.0.1-SNAPSHOT.jar"]
