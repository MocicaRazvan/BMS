...
# ---------------------- USER SERVICE  ----------------------
FROM maven-base AS user-service
...
RUN mvn -f user-service/pom.xml dependency:go-offline -B -q
COPY user-service/src user-service/src
RUN mvn -q -f user-service/pom.xml clean package -DskipTests -Dmaven.compiler.showWarnings=true -Dmaven.compiler.source=22 -Dmaven.compiler.target=22 -Dmaven.compiler.compilerArgs=--enable-preview
...
COPY --from=user-service /app/user-service/target/user-service-0.0.1-SNAPSHOT.jar .
