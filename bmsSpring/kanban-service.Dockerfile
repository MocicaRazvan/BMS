...
# ---------------------- KANBAN SERVICE  ----------------------
FROM maven-base AS kanban-service
COPY --from=template-module /root/.m2 /root/.m2
COPY --from=redis-cache /root/.m2 /root/.m2
COPY kanban-service/pom.xml kanban-service/pom.xml
RUN mvn -f kanban-service/pom.xml dependency:go-offline -B
COPY kanban-service/src kanban-service/src
RUN mvn -q -f kanban-service/pom.xml clean package -DskipTests -Dmaven.compiler.showWarnings=true -Dmaven.compiler.source=22 -Dmaven.compiler.target=22 -Dmaven.compiler.compilerArgs=--enable-preview
...
ENTRYPOINT ["java", "--enable-preview", "-Dreactor.schedulers.defaultBoundedElasticOnVirtualThreads=true","-jar", "kanban-service-0.0.1-SNAPSHOT.jar"]
