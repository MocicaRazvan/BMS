...
COPY --from=build --chown=appuser:appgroup \
     /app/target/service-registry-0.0.1-SNAPSHOT.jar \
     service-registry-0.0.1-SNAPSHOT.jar

USER appuser:appgroup

ENTRYPOINT ["java", "--enable-preview", "-Dreactor.schedulers.defaultBoundedElasticOnVirtualThreads=true","-jar", "service-registry-0.0.1-SNAPSHOT.jar"]