...
RUN mvn -f ingredient-service/pom.xml dependency:go-offline -B -q
COPY ingredient-service/src ingredient-service/src
RUN mvn  -q -f ingredient-service/pom.xml clean package -DskipTests -Dmaven.compiler.showWarnings=true -Dmaven.compiler.source=22 -Dmaven.compiler.target=22 -Dmaven.compiler.compilerArgs=--enable-preview
...
COPY --from=ingredient-service \
     --chown=appuser:appgroup \
     /app/ingredient-service/target/ingredient-service-0.0.1-SNAPSHOT.jar \
     ingredient-service-0.0.1-SNAPSHOT.jar
