...
RUN mvn -f post-service/pom.xml dependency:go-offline -B -q
COPY post-service/src post-service/src
RUN mvn -q -f post-service/pom.xml clean package -DskipTests -Dmaven.compiler.showWarnings=true -Dmaven.compiler.source=22 -Dmaven.compiler.target=22 -Dmaven.compiler.compilerArgs=--enable-preview
