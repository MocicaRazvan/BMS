...
function cleanString(str: string, targetedField: TargetedFields) {
...
export async function POST(req: NextRequest) {
  ...
  const llm = new ChatOllama({
    ...
    numCtx: process.env.OLLAMA_NUM_CTX
      ...
      : 2048,
    streaming: streamResponse !== false,
    callbacks: streamResponse !== false ? [newHandlers] : undefined,
  });
  ...
  const chainPromise = retrievalChain.invoke({
  ...
  if (streamResponse === false) {
    const resp = await chainPromise;
    // console.log(resp);
    // let answer = resp.answer.replace(/```html|```/gi, "");
    // if (targetedField === "title") {
    //   answer = removeMd(answer).replace(/["`]/g, "");
    // }
    // answer = answer.trim();
